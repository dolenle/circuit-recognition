%clear all; close all;
%% Importing Data
function [ decision, p] = KNN_classifier (x_train,y_train,x_test);
...
%                 size(test_data{ii}(jj,:))
%                 size(train_data{kk}(mm,:))
%                 [ii jj kk]
...
if p ==0
    fileID = fopen('KNN_OUT.txt','w');
    fprintf(fileID,'%d, %d\n', [(1:length(decision{1})) ; decision{1}]);
    csvwrite('KNN_OUT.csv',[(1:length(decision{1}))' , decision{1}']);
    fclose(fileID);
end
